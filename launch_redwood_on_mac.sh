#!/bin/bash
#
# launch_redwood_on_mac.sh
# for use with the Mac Mini Subject Machines on OSX
...
./run_on_machines.sh "ID=\`hostname | sed 's/leeps-//' | sed 's/.local//'\`; open -a \"Google Chrome.app\" $URL" $3 &
